...
import numpy as np
from convolve import convolove
...
class Length(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        out = K.permute_dimensions(inputs, (0, 1, 2, 4, 3))
        return K.squeeze(out, axis=4)
...
class LengthOriginal(layers.Layer):
    def __init__(self, num_classes, seg=True, **kwargs):
        super(Length, self).__init__(**kwargs)
        if num_classes == 2:
            self.num_classes = 1
    ...
    def call(self, inputs, **kwargs):
        if type(inputs) is list:
            ...
            if input.get_shape().ndims == 3:
                ...
                masked = tf.squeeze(mask, axis=-1) * input
...
class ConvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        input_tensor_reshaped - [BS * input_num_capsule, h, w, input_num_atom]
        has to be changed to - [BS, h, w, input_num_capsule, input_num_atom]
        
        W - [k, k, input_num_atom, num_atom * num_capsule]
        has to be changed - [k, k, input_num_capsule, input_num_atom,  num_atom * num_capsule] 
        """
        votes, conv_height, conv_width = convolove(input_tensor=input_transposed, kernel=self.W, strides=self.strides,
                                                   padding=self.padding,
                                                   num_caps=self.num_capsule, num_atom=self.num_atoms)
        print(votes)
