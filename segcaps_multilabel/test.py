...
def test(args, test_list, model_list, net_input_shape, i):
    ...
    with open(join(output_dir, args.save_prefix + outfile + str(i) + 'scores.csv'), 'w') as csvfile:
        ...
        for i, img in enumerate(tqdm(test_list)):
            ...
            gt_data_multilabel = [np.double(gt_data[:, :, 0] == i) for i in range(6)]
            # Plot Qual Figure
            ...
            plt.figure(2)
            output_array[0][0, :, :, 0] = output_array[0][0, :, :, 0] / 2
            pred = np.argmax(output_array[0][0, :, :, :], axis=2)
            pred_multilabel = [np.double(pred == i) for i in range(6)]
            plt.imshow(pred/5)
            # plt.pause(0.1)
            plt.savefig(f'output_{img}.png')

            output_bin = np.reshape(output_bin, (output_bin.shape[1], output_bin.shape[2]))
            row = [img[0][:-4]]
            if args.compute_dice:
                print('Computing Dice')
                dice_arr[i] = np.mean(np.array(
                    [dc(pred_multilabel_i, gt_data_multilabel_i) for pred_multilabel_i, gt_data_multilabel_i in
                     zip(pred_multilabel, gt_data_multilabel)]))
