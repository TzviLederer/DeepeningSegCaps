...
def compute_class_weights(root, train_data_list):
    ...
    pos = 0.0
    neg = 0.0
    while ([] in train_data_list):
        train_data_list.remove([])
...
def convert_data_to_numpy(root_path, img_name, no_masks=False, overwrite=False, image_shape=(512, 512)):
    fname = img_name[:-4]
    ...
    try:
        ...
        im = cv2.resize(im, image_shape[:2])
        gray = im[:, :, 0]
        img = np.reshape(gray, (gray.shape[0], gray.shape[1], 1))
        ...
        if not no_masks:
            ...
            mask = cv2.resize(mask, image_shape[:2])
            mask = mask[:, :, 0]
...
def generate_train_batches(root_path, train_list, net_input_shape, net, image_shape, batchSize=1, numSlices=1, subSampAmt=-1,
                           stride=1, downSampAmt=1, shuff=1, aug_data=0, class_num=6):
    # Create placeholders for training
    img_batch = np.zeros((np.concatenate(((batchSize,), net_input_shape))), dtype=np.float32)
    mask_batch = np.zeros((np.concatenate(((batchSize,), (net_input_shape[0], net_input_shape[1], class_num)))), dtype=np.uint8)
    ...
    while True:
        ...
        for i, scan_name in enumerate(train_list):
            try:
                ...
                train_img, train_mask = convert_data_to_numpy(root_path, scan_name, image_shape=image_shape)
            ...
            for j in indicies:
                ...
                if count % batchSize == 0:
                    ...
                    if debug:
                        ...
                        plt.close()
                    if net.find('caps') != -1:
                        yield ([img_batch, mask_batch], [mask_batch, np.expand_dims(np.double(mask_batch!=0)[:,:,:,-1], axis=3) * img_batch])
                    else:
...
def generate_val_batches(root_path, val_list, net_input_shape, net, batchSize=1, numSlices=1, subSampAmt=-1,
    ...
    img_batch = np.zeros((np.concatenate(((batchSize,), net_input_shape))), dtype=np.float32)
    mask_batch = np.zeros((np.concatenate(((batchSize,), (net_input_shape[0], net_input_shape[1], 6)))), dtype=np.uint8)
    ...
    while True:
        ...
        for i, scan_name in enumerate(val_list):
            try:
                ...
                print('\nPre-made numpy array not found for {}.\nCreating now...'.format(scan_name[:-4]))
                val_img, val_mask = convert_data_to_numpy(root_path, scan_name, image_shape=net_input_shape)
...
def generate_test_batches(root_path, test_list, net_input_shape, batchSize=1, numSlices=1, subSampAmt=0,
    ...
    for i, scan_name in enumerate(test_list):
        ...
        try:
            ...
            print('\nPre-made numpy array not found for {}.\nCreating now...'.format(scan_name[:-4]))
            test_img = convert_data_to_numpy(root_path, scan_name, no_masks=True, image_shape=net_input_shape)
