...
def caps_length(x, axis=2):
    x = layers.Lambda(lambda x: tf.transpose(x, (0, 3, 4, 1, 2)))(x)
    pred = layers.Lambda(lambda x: tf.sqrt(tf.reduce_sum(tf.square(x), axis=axis, keepdims=True)))(x)
    pred = layers.Lambda(lambda x: tf.squeeze(x, axis=axis))(pred)
    pred = layers.Lambda(lambda x: tf.transpose(x, (0, 2, 3, 1)), name='out_seg')(pred)
...
def CapsNetR3(input_shape, n_class=2):
    ...
    reshape = ConvCapsuleLayer(kernel_size=1, num_capsule=n_class, num_atoms=16, strides=1, padding='same',
    ...
    masked_by_y = Mask()([out_seg, y])  # The true label is used to mask the output of capsule layer. For training
    masked = Mask()(out_seg)  # Mask using the capsule with maximal length. For prediction

    def shared_decoder(mask_layer):
        recon_remove_dim = layers.Reshape((H.value, W.value, C.value))(mask_layer)
