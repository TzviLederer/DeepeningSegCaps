...
def test(args, test_list, model_list, net_input_shape,i):
    ...
    with open(join(output_dir, args.save_prefix + outfile +str(i)+ 'scores.csv'), 'w') as csvfile:
        ...
        for i, img in enumerate(tqdm(test_list)):
            ...
            cv2.imwrite(join(raw_out_dir, img[0][:-4] + '_raw_output' + img[0][-4:]),output_img[0,:,:].astype('uint8'))
            cv2.imwrite(join(raw_out_dir, img[0][:-4] + '_raw_output' + img[0][-4:]),output_mask[0, :,:].astype('uint8'))
            cv2.imwrite(join(fin_out_dir, img[0][:-4] + '_final_output' + img[0][-4:]),np.reshape(output_mask,(output_img.shape[1],output_img.shape[2])))
